...
lazy3.cpp v3.1, modified by Gerald R. Tamayo with bitsbyte.c (October/05/2025)
...
To compile: g++ -O3 lazy.cpp -static -s -o lazy
...
/* log2(x) */
// return 0..32 bits in x not counting leading 0 bits, with handling of x = 0 
int lg(unsigned x)
{
  if ( x == 0 ) return 1;
...
int main(int argc, char** argv) {
  ...
  if (argc!=4 || (cmd=argv[1][0])!='d' && (cmd<'1' || cmd>'5')) {
    fprintf(stderr,
      ...
      "Licensed under GPL v3. http://www.gnu.org/copyleft/gpl.html\n"
      "lazy3, modified by Gerald R. Tamayo with gtbitio3, August 05, 2024\n"
    ...
    while ((c=gfgetc())!=EOF) {
      ...
      if (state==2 && n>=m) {  // expect m offset bits
        ...
        while (len--)  // output match
          pfputc(buf[ptr++&(BUFSIZE-1)]=buf[p++&(BUFSIZE-1)]);
        bits>>=m;
        n-=m;
        state=0;
      }
      ...
      if (state==4 && n>=8) {  // expect len literals
        assert(len>0);
        pfputc(buf[ptr++&(BUFSIZE-1)]=bits&255);
        bits>>=8;
        n-=8;
        if (--len<1) state=0;
        assert(n<8);
  ...
  printf("%ld -> %ld in %1.2f sec\n", ftell(mode==COMPRESS?in:gIN), ftell(pOUT),
